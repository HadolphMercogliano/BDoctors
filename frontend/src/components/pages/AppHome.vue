...
export default {
  ...
  methods: {
    getData() {
      axios.get(this.store.apiBaseUrl + this.store.apiUrls.doctors).then((response) => {
    ...
    search() {
      if (this.store.searchBarText.trim() !== "" && this.store.selectedSpecializations.length > 0) {
        return this.store.doctors.filter((doc) => {
          if (
            this.store.selectedSpecializations.every((cat) =>
              doc.specializations.map((specializations) => specializations.id).includes(cat)
            )
          ) {
            return (
              doc.name.toLowerCase().includes(this.store.searchBarText.toLowerCase()) ||
              doc.surname.toLowerCase().includes(this.store.searchBarText.toLowerCase())
            );
          }
        });
      } else if (this.store.searchBarText.trim() !== "") {
        return this.store.doctors.filter(
          (doc) =>
            doc.name.toLowerCase().includes(this.store.searchBarText.toLowerCase()) ||
            doc.surname.toLowerCase().includes(this.store.searchBarText.toLowerCase())
        );
  ...
  <main class="pt-5">
    <section class="pb-4 d-flex flex-column flex-sm-row mt-2 container-md container-fluid mx-auto">
      ...
      <div class="doctors col-sm-8 col-md-9 ms-0">
        ...
        <div
          class="ms_cards mt-4 d-flex gap-5 flex-wrap justify-content-center align-content-start">
